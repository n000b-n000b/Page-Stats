...
def main(a000,a001,a002):
    ...
    i.markdown("<h1 style='text-align: center; '> Welcome to Page-Stats !</h1>", unsafe_allow_html=True)
    col00,col001=i.columns([4,5])
    with col00:
        i.caption("<h1 style='text-align: center'>Overview</h1><p style='font-size: 26px; text-align: center'>Page-Stats is a simple and user friendly educational webpage analysis &amp; visualization project. Summarizing the markup language of sources provided in a 3D surface graph. Updates and features are in development. In the mean time enter three URLs and have a look at the data!</p>",unsafe_allow_html=True)
    with col001:
        form=i.form("Main", clear_on_submit=True)
        a000=form.text_input("Enter URL 1 (example.com): ")
        a001=form.text_input("Enter URL 2 (example.com): ")
        a002=form.text_input("Enter URL 3 (example.com): ")
                ...
                try:
                    ...
                    with i.container():
                        ...
                        with i.container():
                            ...
                            x,y= frame00.columns, frame00.index
                            fig00=k.Figure(data=k.Surface(z=z,x=x,y=y))
                            fig00.update_layout(scene=dict(title='Page-Stats!', title_x=0.5, autosize=False,
                                    width=1080, height=900,
                                    margin=dict(l=65, r=50, b=65, t=90,)))
                            i.plotly_chart(fig00)
                except:
